...
fun convertBookshelfToBookshelfDb(bookshelf: Bookshelf): BookshelfDb =
    BookshelfDb(
        bookshelfId = bookshelf.id,
        bookshelfTitle = bookshelf.title
    )

fun convertBookshelfDbToBookshelf(bookshelf: BookshelfDb): Bookshelf =
    Bookshelf(
        id = bookshelf.bookshelfId,
        title = bookshelf.bookshelfTitle
    )

fun convertListBookshelfDbToBookshelf(list: List<BookshelfDb>): List<Bookshelf> =
    list.map { bookshelf ->
        convertBookshelfDbToBookshelf(bookshelf)
    }
